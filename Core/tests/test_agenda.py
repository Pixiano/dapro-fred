# Core/tests/test_agenda.py
#
# Built alongside agenda.py on 2026-08-09: Vatsal named reliable
...
def test_parse_due_date_today_and_tomorrow():
    ...
    assert agenda.parse_due_date("today", now=now).date() == now.date()
    assert agenda.parse_due_date("tomorrow", now=now).date() == (now + timedelta(days=1)).date()


def test_parse_due_date_in_n_days_and_weeks():
    now = datetime(2026, 8, 9, 15, 0)
    assert agenda.parse_due_date("in 3 days", now=now).date() == (now + timedelta(days=3)).date()
    assert agenda.parse_due_date("in a day", now=now).date() == (now + timedelta(days=1)).date()
    assert agenda.parse_due_date("in 2 weeks", now=now).date() == (now + timedelta(days=14)).date()
    assert agenda.parse_due_date("in a week", now=now).date() == (now + timedelta(days=7)).date()
    assert agenda.parse_due_date("next week", now=now).date() == (now + timedelta(days=7)).date()


def test_parse_due_date_weekday_is_next_occurrence_including_today():
    # 2026-08-09 is a Sunday.
    now = datetime(2026, 8, 9, 9, 0)
    assert agenda.parse_due_date("sunday", now=now).date() == now.date()
    assert agenda.parse_due_date("wednesday", now=now).date() == (now + timedelta(days=3)).date()


def test_parse_due_date_iso():
    assert agenda.parse_due_date("2026-09-01").date() == datetime(2026, 9, 1).date()
...
def test_parse_due_date_named_month_day_first_with_year():
    ...
    assert agenda.parse_due_date("13 August 2026").date() == datetime(2026, 8, 13).date()
    assert agenda.parse_due_date("on 13 August 2026").date() == datetime(2026, 8, 13).date()
    assert agenda.parse_due_date("13th August 2026").date() == datetime(2026, 8, 13).date()
    assert agenda.parse_due_date("3 Aug 2026").date() == datetime(2026, 8, 3).date()
...
def test_parse_due_date_named_month_day_first_no_year_infers_forward():
    ...
    assert agenda.parse_due_date("13 August", now=now).date() == datetime(2026, 8, 13).date()
    # Already passed this year: rolls to next year, not into the past.
    assert agenda.parse_due_date("1 January", now=now).date() == datetime(2027, 1, 1).date()


def test_parse_due_date_named_month_month_first():
    assert agenda.parse_due_date("August 13, 2026").date() == datetime(2026, 8, 13).date()
    assert agenda.parse_due_date("Aug 13 2026").date() == datetime(2026, 8, 13).date()
...
def test_parse_due_date_full_month_names_not_truncated_by_abbreviation_match():
    ...
    assert agenda.parse_due_date("5 September 2026").date() == datetime(2026, 9, 5).date()
    assert agenda.parse_due_date("5 June 2026").date() == datetime(2026, 6, 5).date()


def test_parse_due_date_garbage_is_none():
    assert agenda.parse_due_date("") is None
    assert agenda.parse_due_date("whenever") is None
    assert agenda.parse_due_date(None) is None


def test_resolve_when_with_time_uses_scheduler_and_sets_has_time():
    when, has_time = agenda._resolve_when("tomorrow", "2:45pm")
    assert has_time is True
    assert when.hour == 14 and when.minute == 45


def test_resolve_when_without_time_has_no_time_flag():
    when, has_time = agenda._resolve_when("tomorrow", "")
...
def test_add_homework_with_count_reads_back_everything(tmp_path, monkeypatch):
    ...
    result = agenda.add_item("homework", "Geography", detail="3 questions", count=3, due="tomorrow")
...
def test_add_homework_without_count_has_no_progress_phrase(tmp_path, monkeypatch):
    ...
    result = agenda.add_item("homework", "English", detail="an essay", due="friday")
...
def test_add_event_with_prep_and_time(tmp_path, monkeypatch):
    ...
    result = agenda.add_item("event", "Movie", due="today", time="2:45pm", prep_minutes=30)
...
def test_add_event_with_prep_but_no_time_is_rejected(tmp_path, monkeypatch):
    ...
    result = agenda.add_item("event", "Movie", due="today", prep_minutes=30)

    assert "start time" in result.lower()
    assert agenda._load_items() == []  # nothing written on rejection


def test_add_item_unknown_kind_rejected(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    result = agenda.add_item("chore", "Wash the car", due="tomorrow")
    assert "isn't something i track" in result.lower()


def test_add_item_missing_subject_rejected(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    assert "subject" in agenda.add_item("homework", "", due="tomorrow").lower()


def test_add_item_unparseable_due_rejected(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    result = agenda.add_item("homework", "Geography", due="sometime maybe")
    assert "couldn't work out when" in result.lower()
    assert agenda._load_items() == []


def test_zero_count_and_zero_prep_are_treated_as_absent(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", count=0, due="tomorrow")
    item = agenda._load_items()[0]
...
def test_repeated_saves_do_not_accumulate_blank_lines(tmp_path, monkeypatch):
    ...
    agenda.add_item("homework", "Physics", due="tomorrow")
    agenda.delete_item("physics")
    agenda.update_item("geography", note="still stable")

    text = (tmp_path / "agenda.md").read_text(encoding="utf-8")
    assert "# Agenda\n\n## Items" in text
    assert "# Agenda\n\n\n## Items" not in text
...
def test_list_items_filters_by_when(tmp_path, monkeypatch):
    ...
    agenda.add_item("homework", "Overdue thing", due=overdue_iso)
    agenda.add_item("homework", "Today thing", due="today")
    agenda.add_item("homework", "Tomorrow thing", due="tomorrow")
    agenda.add_item("homework", "This week thing", due="in 3 days")
    agenda.add_item("homework", "Far future thing", due=far_iso)

    today_list = agenda.list_items(when="today")
    assert "Today thing" in today_list
    assert "Tomorrow thing" not in today_list

    tomorrow_list = agenda.list_items(when="tomorrow")
    assert "Tomorrow thing" in tomorrow_list
    assert "Today thing" not in tomorrow_list

    week_list = agenda.list_items(when="week")
    assert "This week thing" in week_list
    assert "Far future thing" not in week_list

    overdue_list = agenda.list_items(when="overdue")
    assert "Overdue thing" in overdue_list
    assert "Today thing" not in overdue_list

    all_list = agenda.list_items(when="all")
...
def test_list_items_filters_by_kind_and_subject(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", due="tomorrow")
    agenda.add_item("project", "Physics model", due="tomorrow")

    only_homework = agenda.list_items(kind="homework")
    assert "Geography" in only_homework
    assert "Physics model" not in only_homework

    only_geo = agenda.list_items(subject="geography")
...
def test_list_items_excludes_done(tmp_path, monkeypatch):
    ...
    assert "Geography" not in agenda.list_items()


def test_list_items_empty_says_so(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    assert "nothing" in agenda.list_items(when="tomorrow").lower()
...
def test_add_progress_partial_stays_open(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", count=3, due="tomorrow")

    result = agenda.update_item("geography", add_progress=2)
...
def test_add_progress_reaching_total_auto_completes(tmp_path, monkeypatch):
    ...
    agenda.update_item("geography", add_progress=2)
    agenda.update_item("geography", add_progress=1)
...
def test_add_progress_is_clamped_to_total(tmp_path, monkeypatch):
    ...
    agenda.update_item("geography", add_progress=99)
...
def test_set_progress_is_absolute_not_additive(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", count=5, due="tomorrow")
    agenda.update_item("geography", add_progress=4)

    agenda.update_item("geography", set_progress=1)

    assert agenda._load_items()[0]["done_count"] == 1


def test_progress_on_uncounted_item_marks_it_done(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "English", detail="an essay", due="tomorrow")

    agenda.update_item("english", add_progress=1)

    assert agenda._load_items()[0]["done"] is True
...
def test_done_param_overrides_and_is_reversible(tmp_path, monkeypatch):
    ...
    agenda.update_item("geography", done=False)
    assert agenda._load_items()[0]["done"] is False
...
def test_reschedule_updates_when_and_has_time(tmp_path, monkeypatch):
    ...
    result = agenda.update_item("geography", new_due="2026-12-25", note="teacher extended it")
...
def test_reschedule_with_bad_date_is_rejected_and_item_untouched(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", due="tomorrow")
    before = agenda._load_items()[0]["when"]

    result = agenda.update_item("geography", new_due="nonsense")

    assert "couldn't work out" in result.lower()
    assert agenda._load_items()[0]["when"] == before


def test_update_next_step(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("project", "Physics model", count=4, due="in 10 days", next_step="buy card sheet")

    agenda.update_item("physics", next_step="cut the base")

    assert agenda._load_items()[0]["next_step"] == "cut the base"


def test_update_no_match_reports_it(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    result = agenda.update_item("nonexistent subject", done=True)
...
def test_update_with_no_fields_given_reports_it(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", due="tomorrow")
    result = agenda.update_item("geography")
...
def test_delete_removes_the_matching_item(tmp_path, monkeypatch):
    ...
    result = agenda.delete_item("geography")

    assert "geography" in result.lower()
    remaining = [i["subject"] for i in agenda._load_items()]
    assert remaining == ["Physics"]


def test_delete_no_match_reports_it(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    result = agenda.delete_item("nonexistent")
    assert "nothing matching" in result.lower()
...
def test_delete_refuses_an_ambiguous_match_rather_than_guessing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", detail="map work", due="tomorrow")
    agenda.add_item("homework", "Geography", detail="questions", due="in 5 days")

    result = agenda.delete_item("geography")

    assert "more than one" in result.lower()
    assert len(agenda._load_items()) == 2  # nothing deleted


def test_delete_empty_match_is_rejected(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", due="tomorrow")
    result = agenda.delete_item("")
    assert "need something to match" in result.lower()
    assert len(agenda._load_items()) == 1


def test_update_picks_the_soonest_due_among_multiple_matches(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    agenda.add_item("homework", "Geography", detail="map work", due="in 10 days")
    agenda.add_item("homework", "Geography", detail="questions", due="tomorrow")
...
def test_due_within_includes_overdue(tmp_path, monkeypatch):
    ...
    agenda.add_item("homework", "Old thing", due=overdue_iso)
...
def test_events_needing_prep_fires_inside_the_window_only(tmp_path, monkeypatch):
    ...
    # "now" sits inside it.
    due = agenda.events_needing_prep(now=datetime.now())
    assert [i["subject"] for i in due] == ["Movie"]

    # Well before the window opens.
    assert agenda.events_needing_prep(now=start - timedelta(hours=3)) == []

    # After the event has already started.
    assert agenda.events_needing_prep(now=start + timedelta(minutes=1)) == []


def test_events_needing_prep_ignores_events_without_prep_or_time(tmp_path, monkeypatch):
    monkeypatch.setattr(agenda, "VAULT_DIR", tmp_path)
    start = _fixed(minutes=30)
    agenda.add_item("event", "No-prep thing", due=start.strftime("%Y-%m-%d"), time=start.strftime("%H:%M"))
    agenda.add_item("event", "No-time thing", due="tomorrow", prep_minutes=30)

    assert agenda.events_needing_prep(now=datetime.now()) == []
...
def test_events_upcoming_within_window(tmp_path, monkeypatch):
    ...
    soon = _fixed(hours=5)
    far = _fixed(hours=48)
    agenda.add_item("event", "Soon event", due=soon.strftime("%Y-%m-%d"), time=soon.strftime("%H:%M"))
    agenda.add_item("event", "Far event", due=far.strftime("%Y-%m-%d"), time=far.strftime("%H:%M"))

    upcoming = agenda.events_upcoming(within_hours=24)
...
def test_carryover_candidates_only_due_today_or_earlier_and_open(tmp_path, monkeypatch):
    ...
    agenda.add_item("homework", "Today thing", due="today")
    agenda.add_item("homework", "Tomorrow thing", due="tomorrow")
    agenda.add_item("event", "Today event", due="today", time="6pm")
