...
AUTHORIZED_USERS = ["Vatsal", "Suhani", "Jiyana", "Anvay", "Vatsal Da Pro", "Devansh", "Pranati", "Saloni", "Vihaan", "Dhiraj"]
...
def persistent_remember(role, content, embedding=None):
    ...
    mem.append({"role": role, "content": content, "embedding": embedding, "timestamp": None})
...
def search_memory_enhanced(keyword):
    ...
    filtered = [r for r in results if keyword.lower() in r['content'].lower()]
...
@app.route("/")
def index():
    print("[DEBUG] Serving index_real.html")
...
def handle_message(data):
    ...
    print(f"\n[DEBUG] Received message from {username}: {user_input}")
    ...
    if user_input.lower() in ["exit", "quit"]:
        print("[DEBUG] Exit command received. Shutting down...")
        emit("fred_reply", {"text": "Shutting down server...", "username": username}, broadcast=True)
    ...
    def process_fred_reply():
        ...
        if fred_reply.strip().split(" ")[0].lower() in OS_KEYWORDS and username not in OS_ALLOWED_USERS:
        ...
        print(f"[DEBUG] Emitted reply: {fred_reply[:80]}...")
...
# -----------------------
def run_server():
    print("🚀 F.R.E.D. LAN + ngrok demo starting...")
